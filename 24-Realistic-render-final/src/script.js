...
const createEnvironmentLighting = (scene, fileName) => {
    // scene.clearColor = new BABYLON.Color3.FromInts(235, 235, 235);
    var envLighting = BABYLON.CubeTexture.CreateFromPrefilteredData(fileName, scene);
    envLighting.gammaSpace = false;
    // envLighting.groundShadowLevel = 0.7;
    scene.environmentTexture = envLighting;

    // // Create a default skybox with an environment.
    // scene.createDefaultCameraOrLight(true, true, true);
    // var hdrTexture = BABYLON.CubeTexture.CreateFromPrefilteredData("studio.env", scene);
